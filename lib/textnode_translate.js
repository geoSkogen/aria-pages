...
function translate_text(str_arr, lang, dom_els) {
  ...
  function restore_textnodes(dom_els, new_strs) {
    for (i = 0; i < dom_els.length; i++) {
      dom_els[i].el.appendChild( document.createTextNode( new_strs[i]))
    }
  }

  const xhttp = new XMLHttpRequest()
  const url = new URL('http://localhost/psu-translate/translation-service/index.php')
  var query_string = '?lang=' + lang + '&'
  var resp

  for (let i = 0; i < str_arr.length; i++) {
    query_string += 'content_' + i.toString() + '=' + str_arr[i]
    query_string += (i==str_arr.length-1) ?
      '' : '&'
  }

  xhttp.open("GET", "http://localhost/psu-translate/translation-service/index.php" + query_string, true)
  xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  console.log("http://localhost/psu-translate/translation-service/index.php" + query_string)
...
translate_text(dom_obj.text_strs, lang, dom_obj.text_els)
