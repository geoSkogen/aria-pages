...
function renderTableHeader(data,columnwise_atts) {
  ...
  for (let i = 0; i < column_names.length; i++) {
    ...
    if (Object.keys(columnwise_atts).indexOf(cell_val) > -1) {
      ...
      if (button_span.className.indexOf('aria-sort')>-1) {
...
function dataGridNavigation(event) {
  ...
  switch(event.code) {
    ...
    case 'Space' :
      ...
      if (current_cell.getAttribute('aria-sort')) {
        toggleTableSort(current_cell.querySelector('.aria-sort-data-grid-button'),TABLE_DATA)
      }
      next_cell = current_cell
      break
    default :
...
function toggleTableSort(event_target_element,raw_table_data) {
  ...
  let sort_state_index = sort_states.indexOf(event_target_element.parentElement.getAttribute('aria-sort'))
  let sorted_data = []
  var column_attr_props = {}
  var data_column_name = event_target_element.parentElement.getAttribute('data-column-name')
...
const TABLE_DATA = TABLE_DATASET[data_selection]
